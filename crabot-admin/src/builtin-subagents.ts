 ...
 *
 * 三个 builtin：code_planner / code_writer / vision。
...
const VISION_WHEN_TO_USE = `Use this subagent when:
- 用户消息含图片且需要理解图片内容
- 需要分析截图 / UI 界面 / 数据图表

<example>
Context: 用户发来截图问"这页面布局有什么问题？"
user: [图片] 这个布局有啥问题？
assistant: 调用 delegate_task(subagent_type="vision", task="分析截图的视觉层级和布局问题", image_paths=["..."])
<commentary>截图分析任务，明显的视觉理解需求</commentary>
</example>`

const VISION_ROLE = `你是视觉分析专家。
...
const VISION_WORKFLOW = `1. 仔细观察图片/截图
2. 识别主要元素 / 布局结构 / 文本内容
3. 综合分析后形成结论`

const VISION_DELIVERABLES = `返回 markdown 格式分析报告，含：
- 简要描述图片内容（1-2 句）
- 关键发现（bullet list 3-5 条）
- 改进建议（如适用）
- 长度：200-500 字`

const VISION_VERIFICATION = `返回前自检：
- 是否真的看清楚了图片（不要硬猜）
- 改进建议是否具体（避免"应该更好"空话）

若无法完成：
- 图片不清晰 → 说明原因，不要硬猜
- 任务超出视觉分析范围 → 拒绝并建议改用其他 subagent`
...
export function getBuiltinSubAgents(): SubAgentRegistryEntry[] {
  return [
    ...
    {
      id: BUILTIN_SUBAGENT_IDS.vision,
      name: 'vision',
      description: '视觉分析专家：截图分析 / UI 识别 / 图片内容理解',
      when_to_use: VISION_WHEN_TO_USE,
      role: VISION_ROLE,
      workflow: VISION_WORKFLOW,
      deliverables: VISION_DELIVERABLES,
