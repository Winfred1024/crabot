...
| Channel-Host (Shim) | `crabot-channel-host` | TS | 19003+ | ✅ | ✅ |
...
| OpenClaw shim | `channel-host` | `crabot-channel-host` 加载 OpenClaw 插件（如 `@tencent-weixin/openclaw-weixin`） | OpenClaw 插件自管 |

新 channel 优先原生模块，shim 不稳定。
