...
export const PLAN_AND_EXECUTE_GUIDE = `
## Plan-and-Execute 协作模式

当你接到"修改 / 重构 / 新增功能"类编码需求且涉及 2 个或更多文件、或需要选型 / 多步骤实施时，**强烈建议**走以下两步串联：

1. 先调 \`delegate_task(subagent_type="code_planner", task=...)\` 拿到 plan 文件路径
2. 再调 \`delegate_task(subagent_type="code_writer", task="按 <plan_path> 实施所有 task")\`

不要自己直接写代码！让 code_planner（用强模型）做规划 + 拆解，code_writer（用性价比模型）按 plan 照办。这能省成本 + 加速 + 让 plan 可审计。
