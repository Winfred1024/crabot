...
### 三个合法场景（intent 参数，全部 audience 都是人类）

**给用户发回复必须显式调用 \`send_message\` 工具**——不会自动回复。完成任务的最终交付也是 \`send_message\`，发完直接 end_turn 即可。

- \`intent="info"\`（默认）：进度告知 / ack / 中间结果。人类看到、不期待回复。发完继续后续工作
- \`intent="final"\`：**本任务的最终交付**。发完即代表你完成了任务，应直接 end_turn 退出 loop。引擎据此判断 silent end_turn 是正常完成
...
**避免错用**：发完最终交付后又 end_turn 但用了 \`intent="info"\` → 引擎会以为你还有工作没做完、误触 forced_summary 机制再要你重说一次 → 用户收到重复消息。**最后那条消息一定用 \`intent="final"\`**。
...
- 目标是 cleared → 人类清掉了你的目标。重新 set_task_goal 写新承诺，或 send_message(intent='final', '总结') 收尾
