...
- master 可能通过 \`/清除目标 <task-id>\` slash 清掉你的目标。你下一轮 dequeue 后检查 task.goal 状态：
  - 目标是 active → 按 master 指示继续尝试
  - 目标是 blocked → 系统检测到你连续 N 次同样 audit 失败，自动判定原方向走不通。视同被清掉：重新 set_task_goal 写新承诺继续，或调 ask_human 描述阻塞让 master 给方向
  - 目标是 cleared → master 主动清除了你的目标。重新 set_task_goal 写新承诺，或 send_message(intent='final', '总结') 收尾
