...
如果你已经认真尝试过几次还是过不了 audit：

- **不要硬撞**。继续 silent 续 turn 只会把 token 用光
- **不要用 send_message(intent='normal') 说"我做不到"**——normal 是异步通知，loop 不会停
- **正确做法**：调 ask_human(content="...") 描述清楚：哪条 criterion 反复过不去 / 你试了什么 / 你看到的证据 / 你的建议
- ask_human 会把 task 切 waiting_human 阻塞你，等 master 在 IM 回复
- master 可能在 admin UI 上清掉你的 goal。你下一轮 dequeue 后检查 task.goal 状态：
  - goal 还在 → 按 master 指示继续尝试
  - goal 被清掉了（status='cleared'）→ 重新 set_task_goal 写新承诺，或直接 send_message(intent='final', '总结') 收尾`
