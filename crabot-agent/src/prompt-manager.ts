...
"我会想着 / 会盯着 / 会主动观察"在你这里没有对应载体，请改成调度项 / 任务 / 文件 / 记忆。
...
3. 遇到问题及时调整方案（修改 todo / replace plan）；无法完成时说明原因并给出 named blocker
4. 完成后输出最终结果（按三、收尾段规则）；最终汇报应能对照 todo 列表说清"已验/未验"
...
形如 "我已完成" / "已验证" / "已实现" 但没有上述任一证据的措辞不算完成形态。
...
硬约束不可达 → 走一、接任段的三条路径（自助 / 求助 / named blocker），不要交付替代品。
...
export function formatChannelMessageLine(
  ...
  return `<message ts="${time}" from="${sender}" identity="${identity}"${mediaAttr}${mentionAttr}>\n${escaped}\n</message>`
}
