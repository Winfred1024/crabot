...
export interface SubmitAuditResultInput {
  /** 整体判决：人类的所有有效要求都满足 = true；任一项不满足 = false */
  pass: boolean
  /** 未满足项 id 列表：worker criterion id 或 req-<slug>（计划未覆盖的人类要求）；pass=true 时应为空数组 */
  failed_criteria: string[]
  /** 逐项核对的证据汇总（markdown）；首段为要求提炼清单，后接每项的工具证据 + 判定理由 */
  evidence: string
}
...
参数：
- pass: 人类的所有有效要求都满足 = true；任一项不满足 = false。拿不准 → false。
- failed_criteria: 未满足项的 id 列表（pass=true 时空数组）。对应 worker 某条 criterion 时填该 id；是 worker 计划没覆盖的人类要求时起稳定 slug（req-<简短描述>），同一缺口多轮审计间用同一 slug。
- evidence: markdown，首段为"本次任务的有效要求"提炼清单（每项注明出处），后接逐项核对——每项说清楚"用了什么工具 / 采到的证据片段 / 判定结果"。worker 续作时会看这份证据。
  ...
  return {
    ...
    inputSchema: {
      ...
      properties: {
        pass: {
          type: 'boolean',
          description: '整体判决；人类的所有有效要求都满足才 true',
        },
        failed_criteria: {
          type: 'array',
          items: { type: 'string' },
          description: '未满足项 id 列表（criterion id 或 req-<slug>）；pass=true 时空数组',
        },
        evidence: {
          type: 'string',
          description: 'markdown 证据汇总；首段为要求提炼清单，后接逐项核对',
        },
      },
